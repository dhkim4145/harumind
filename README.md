...
### 📱 PWA (홈 화면 설치 지원)
- **설치 가능한 앱**: 브라우저에서 홈 화면에 직접 설치
  - Chrome, Edge, Firefox 등 모던 브라우저 지원
  - iOS Safari에서도 웹 클립(홈 화면 추가) 가능
- **오프라인 지원**: 설치 후 인터넷 없이 게임 플레이 가능 (준비 중)
- **네이티브 앱처럼**: 시작 화면 없이 게임에 바로 진입
  - 앱 아이콘: 감성 있는 하루마음 로고
  - 앱 테마: 어두운 배경으로 세련된 느낌 유지
  - 상태바 색상: 테마와 조화된 컬러
- **빠른 로딩**: 캐시된 리소스로 즉각적인 게임 시작
- **설치 방법**:
  - PC: 주소창 오른쪽 설치 버튼 클릭
  - 모바일: 메뉴 → "앱 설치" 또는 "홈 화면에 추가"
